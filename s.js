async function replyToLineMessage(replyToken) {
    ...
    async function fetchData(pageSize, startAfter) {
      ...
      let query = db.collection('2024-04-17').orderBy('Time').limit(pageSize);
      if (startAfter) {
    ...
    async function createPDF(data) {
      ...
      for (let item of data) {
        ...
        body.push([
          ...
          { text: item.name_Helmet || '', alignment: 'center' },
      ...
      let pdfFilename = 'data_report.pdf';
    ...
    try {
      let pageSize = 10; // จำนวนข้อมูลที่จะดึงในแต่ละครั้ง
      let startAfter = null;
      let allData = [];
      do {
        const { data, lastVisible } = await fetchData(pageSize, startAfter);
        allData.push(...data);
      ...
      const filename = await createPDF(allData);
...
async function createPDF() {
    ...
    let body = [];
    let rowNum = 1; // ตัวแปรสำหรับเก็บค่าลำดับแถว
    for (let item of data) {
    let downloadUrl = item.download_url;
    let imageUrl = downloadUrl ? await downloadImage(downloadUrl) : null;
    let licenseUrl = item.url_license;
    ...
    body.push([
        { text: rowNum.toString(), alignment: 'center' }, // เพิ่มเลขลำดับ
        { image: imageUrl || '', fit: [100, 100], alignment: 'center' },
        { image: licenseImageUrl || '', fit: [100, 100], alignment: 'center' },
    ...
    content: [
        { text: 'รถทั้งหมดในวันนี้', style: 'header' },
        { table: { 
        widths: [25, 100, 100, 60, 100, 75], // เพิ่มความกว้างของคอลัมน์แรก
        body: [
            ['No.', 'Image', 'License', 'Name Helmet', 'ID',  'Time'], // เพิ่มหัวข้อ 'No.'
            ...body
        ] 
        } }
    ],
